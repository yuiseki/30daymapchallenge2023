...
export const getValhallaResponse = async (
  departure: LonLat,
  destination: LonLat
) => {
  const json = {
  ...
  const nominatimUrl = `https://valhalla1.openstreetmap.de/route?${params.toString()}`;
...
export const getValhallaResponseJson = async (
  ...
  const res = await getValhallaResponse(departure, destination);
...
export const getValhallaResponseJsonWithCache = async (
  ...
  const md5 = new Md5();
  md5.appendStr(JSON.stringify({ departure, destination }));
  const hash = md5.end();
  const key = `30daymapchallenge2023-valhalla-cache-${hash}`;
